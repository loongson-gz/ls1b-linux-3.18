...
{
	const struct goodix_platform_data *pdata;
	...
	if (ts->gpiod_int && ts->gpiod_rst) {
		...
		ts->gpio_int = pdata->irq_pin;
		ts->gpio_rst = pdata->reset_pin;

		ts->swapped_x_y = pdata->swapped_x_y;
		ts->inverted_x = pdata->inverted_x;
		ts->inverted_y = pdata->inverted_y;
	}
